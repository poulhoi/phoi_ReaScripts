--[[
@description phoi_Move item left edge to previous item end
@author Poul Høi
@links 
...
-- USER CONFIG
local allowMoveToStart = true

-- NAME
local scriptName = "phoi_Move item left edge to previous item end"
	...
	for i = 1, #items do
		...
		if itPrev ~= nil then -- if previous item is on the same track 
			...
			if diff > 0 then
				...
				reaper.ApplyNudge(0, 0, 0, 1, diff, true, 0) -- nudge position left by difference
			end
		elseif allowMoveToStart then
			local posIt = reaper.GetMediaItemInfo_Value(it, "D_POSITION")
			if posIt > 0 then
				setItemSelected(it, true)
				reaper.ApplyNudge(0, 0, 0, 1, posIt, true, 0) -- nudge position left to start
			end
		end
	end
