--[[
@description phoi_Move item right edge to next item start
@author Poul Høi
@links 
...
-- USER CONFIG
local allowMoveToEnd = true

-- NAME
local scriptName = "phoi_Move item right edge to next item start"
...
function main()
	...
	for i = #items, 1, - 1 do -- counts backwards for correct order of item processing
		local it = items[i]
		...
		if itNext ~= nil then -- if next item is on the same track 
			...
			if diff > 0 then
				...
				reaper.ApplyNudge(0, 0, 0, 1, diff, false, 0) -- nudge position by difference
			end
		elseif allowMoveToEnd then
			local endIt = reaper.GetMediaItemInfo_Value(it, "D_POSITION") + reaper.GetMediaItemInfo_Value(it, "D_LENGTH")
